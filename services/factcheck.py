...
class FactChecker:
    ...
    def __init__(self):
        ...
        # FRED series mapping for economic data with units
        self.fred_series = {
        ...
        # Known series units to avoid API calls when possible
        self.fred_units = {
            'UNRATE': 'percent',
            'CPIAUCSL': 'index',  # CPI is an index, not percent
            'GDP': 'billions',
            'GDPC1': 'billions',
            'DFF': 'percent',
            ...
            'SP500': 'index points',
            'DJIA': 'index points',
            'NASDAQCOM': 'index points',
            'FEDMINNFRWG': 'dollars per hour',
            'PAYEMS': 'thousands',
            'JTSJOL': 'thousands',
            'RSXFS': 'millions',
            'UMCSENT': 'index',
    ...
    async def _check_fred_data(self, claim: str) -> Dict:
        ...
        try:
            ...
            # Extract monetary value and type from claim
            claimed_value, claim_units = self._extract_monetary_value(claim)
            ...
            # Get series metadata to understand units
            ...
            async with aiohttp.ClientSession() as session:
                # Get metadata first
                async with session.get(metadata_url, params=metadata_params, timeout=aiohttp.ClientTimeout(total=10)) as meta_response:
                    metadata = {}
                    if meta_response.status == 200:
                        meta_data = await meta_response.json()
                        if meta_data.get('seriess'):
                            metadata = meta_data['seriess'][0]
                ...
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status == 200:
                        ...
                        if data.get('observations'):
                            ...
                            # Determine units based on series and metadata
                            units = self._determine_fred_units(series_id, metadata)
                            
                            # Convert values to same scale for comparison
                            actual_display = actual_value
                            claimed_display = claimed_value
                            
                            # Handle GDP and other dollar-denominated series
                            if units == 'billions' and claim_units == 'dollars':
                                # FRED gives us billions, claim is in raw dollars
                                if claimed_value > 1_000_000_000_000:  # Likely trillions
                                    actual_display = actual_value / 1000  # Convert to trillions
                                    claimed_display = claimed_value / 1_000_000_000_000
                                    units = 'trillion dollars'
                                elif claimed_value > 1_000_000_000:  # Likely billions
                                    claimed_display = claimed_value / 1_000_000_000
                                    units = 'billion dollars'
                                else:
                                    actual_display = actual_value * 1_000_000_000  # Convert to raw dollars
                            elif units == 'thousands' and claim_units == 'dollars':
                                # Employment data in thousands
                                if claimed_value > 1_000_000:  # Likely millions
                                    actual_display = actual_value / 1000
                                    claimed_display = claimed_value / 1_000_000
                                    units = 'million'
                                else:
                                    claimed_display = claimed_value / 1000
                                    units = 'thousand'
                            
                            # Compare values on same scale
                            if units in ['billions', 'billion dollars', 'millions', 'million dollars', 
                                        'thousands', 'thousand', 'trillion dollars']:
                                # For large numbers, use the display values for comparison
                                difference = abs(actual_display - claimed_display)
                                percentage_diff = (difference / actual_display) * 100 if actual_display != 0 else 100
                            else:
                                # For percentages and other units, use raw values
                            ...
                            # Format explanation based on data type
                            if units == 'percent':
                                if verdict == 'true':
                                    explanation = f"✓ Verified: {term} is {actual_value:.1f}% as of {date}"
                                elif verdict == 'false':
                                    explanation = f"✗ Incorrect: {term} is actually {actual_value:.1f}%, not {claimed_value:.1f}%"
                                else:
                                    explanation = f"Federal Reserve data shows {term} is {actual_value:.1f}% as of {date}"
                            else:
                                # Format numbers appropriately using display values
                                actual_formatted = self._format_number_with_units(actual_display, units)
                                claimed_formatted = self._format_number_with_units(claimed_display, units)
                                
                                if verdict == 'true':
                                    explanation = f"✓ Verified: {term} is {actual_formatted} as of {date}"
                                elif verdict == 'false':
                                    explanation = f"✗ Incorrect: {term} is actually {actual_formatted}, not {claimed_formatted}"
                                else:
                                    explanation = f"Federal Reserve data shows {term} is {actual_formatted} as of {date}"
    ...
    def _determine_fred_units(self, series_id: str, metadata: Dict) -> str:
        """Determine the units for a FRED series"""
        
        # First check our known units mapping
        if series_id in self.fred_units:
            return self.fred_units[series_id]
        
        # Check metadata for units
        if metadata:
            units_str = metadata.get('units', '').lower()
            if 'percent' in units_str or '%' in units_str:
                return 'percent'
            elif 'billions' in units_str:
                return 'billions'
            elif 'millions' in units_str:
                return 'millions'
            elif 'thousands' in units_str:
                return 'thousands'
            elif 'dollars' in units_str:
                return 'dollars'
            elif 'index' in units_str:
                return 'index'
        
        # Default
        return 'units'
    
    def _format_number_with_units(self, value: float, units: str) -> str:
        """Format a number with appropriate units"""
        
        if units == 'percent':
            return f"{value:.1f}%"
        elif units == 'billions' or units == 'billion dollars':
            if value >= 1000:
                # Convert to trillions for readability
                return f"${value/1000:,.2f} trillion"
            return f"${value:,.1f} billion"
        elif units == 'millions' or units == 'million dollars':
            if value >= 1000:
                # Convert to billions for readability
                return f"${value/1000:,.1f} billion"
            return f"${value:,.1f} million"
        elif units == 'trillion dollars':
            return f"${value:,.2f} trillion"
        elif units == 'thousands' or units == 'thousand':
            if value >= 1000:
                # Convert to millions for readability
                return f"{value/1000:,.1f} million"
            return f"{value:,.0f} thousand"
        elif units == 'million':
            return f"{value:,.1f} million"
        elif units == 'dollars':
            return f"${value:,.2f}"
        elif units == 'dollars per gallon':
            return f"${value:.2f} per gallon"
        elif units == 'dollars per barrel':
            return f"${value:.2f} per barrel"
        elif units == 'dollars per hour':
            return f"${value:.2f} per hour"
        elif units == 'index points':
    ...
    def _extract_monetary_value(self, claim: str) -> Tuple[Optional[float], Optional[str]]:
        """Extract monetary values from claims, handling different formats"""
        claim_lower = claim.lower()
        
        # Pattern for monetary values
        patterns = [
            # $XX trillion/billion/million
            r'\$\s*(\d+\.?\d*)\s*(trillion|billion|million|thousand)?',
            # XX trillion/billion/million dollars
            r'(\d+\.?\d*)\s*(trillion|billion|million|thousand)?\s*dollars',
            # Just numbers with trillion/billion/million
    ...
    def calculate_credibility(self, fact_checks: List[Dict]) -> float:
        ...
        return round(credibility, 1) in claim:
                    return value, 'dollars'
